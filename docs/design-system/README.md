...
2026-09-16：全セクションはSkillsと同じ初回表示フェードイン（80px上昇・2秒、動きを減らす設定を尊重）。セクション見出し36px/30px、副見出し20px/16pxを基本とする。Landingのサイト名は独立したh1として従来サイズを維持。

2026-09-16 Career改訂：最大幅768px、左寄せ見出し、縦線と丸印の年表、下部の詳細リンク。現在ラベルは他の期間と同じ文字色で表示し、背景色は付けない。モバイルでは期間と説明を縦に並べる。

Career詳細は5項目の縦線付き年表。期間・見出し・役割・説明・主要技術を縦に配置し、見出し20px（モバイル18px）、項目間56pxを確保する。本文は画面高さに制限せず自然に流す。

Career詳細のリードはテーマ色のマーカーを表示。Blogの「すべての記事」リンク上の余白は72px。

2026-09-26の変更：記事本文で段落が URL 1つだけのときはリンクカード(`.link-card`。題名・説明・サイト名、右に OGP 画像。375px では画像が上に積まれる)。OGP は静的生成時に取得し、取れなければ通常のリンク。
